...
class RefactorCodeAction : AnAction() {
    ...
    private fun handleDialogOk(event: AnActionEvent, project: Project, editor: Editor) {
        val settings = AppSettingsState.instance
        val questionText = StringBuilder(settings.doRefactorPrompt)
        ...
        runBlocking {
            if (sendCodeMethod == AppSettingsState.SendCodeMethod.SEND_A_FILE) {
                ...
                ActionGptRequestHelper.makeGPTRequest(project, questionText.toString(), "Refactoring Code...") { text ->
    ...
    private suspend fun uploadFilesAndAskGpt(
        project: Project,
        editor: Editor,
        questionText: String,
        ...
        ActionGptRequestHelper.makeGPTRequest(project, referencedQuestion, "Refactoring Code...") { text ->
            insertResponseIntoEditor(
                project,
                editor,
                text ?: "Nothing was returned from GPT after the request was sent."
            )
        }
    }
